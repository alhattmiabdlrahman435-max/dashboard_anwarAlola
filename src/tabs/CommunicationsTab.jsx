...
export default function CommunicationsTab() {
  const { lang, t, triggerConfirm, canAction, setToastMessage } = useApp();
  ...
  const { page, perPage, search, setPage, setPerPage, setSearch } =
    usePagination({
  ...
  const [modalNotificationStudentId, setModalNotificationStudentId] =
    useState("");
  const [modalNotificationGrade, setModalNotificationGrade] = useState("");
  const [modalNotificationTeacherId, setModalNotificationTeacherId] =
    useState("");
  ...
  const selectedGrade = hasSelectedGrade
    ? modalNotificationGrade
  ...
  const hasSelectedStudent =
    ...
    students.some((s) => String(s.id) === String(modalNotificationStudentId));
  const selectedStudentId = hasSelectedStudent
    ? modalNotificationStudentId
    : students && students.length > 0
      ? students[0].id
      : "";

  const hasSelectedTeacher =
    Array.isArray(teachers) &&
    teachers.some((t) => String(t.id) === String(modalNotificationTeacherId));
  const selectedTeacherId = hasSelectedTeacher
    ? modalNotificationTeacherId
  ...
  const onDeleteAllNotificationsClick = () => {
    triggerConfirm({
      ...
      message:
        lang === "ar"
          ? "هل أنت متأكد من حذف جميع الإشعارات نهائياً؟ هذا الإجراء لا يمكن التراجع عنه!"
          : "Are you sure you want to delete all notifications permanently? This action cannot be undone!",
      onConfirm: () => {
        handleDeleteAllNotifications();
      },
    });
  ...
  const handleCopyContent = (e, notif) => {
    ...
    if (setToastMessage) {
      setToastMessage(
        lang === "ar"
          ? "تم نسخ نص الإشعار بنجاح"
          : "Notification content copied",
      );
    }
  ...
  const applyPresetTemplate = (templateKey) => {
    if (templateKey === "absence") {
      setModalNotificationTitle(
        lang === "ar" ? "تنبيه غياب وتأخر دراسي" : "Absence & Attendance Alert",
      );
      setModalNotificationContent(
        lang === "ar"
          ? "نود إحاطتكم بحضور ومواظبة الطالب/الطالبة، نرجو المتابعة الحثيثة والتواصل مع إدارة المدرسة لضمان التفوق."
          : "We would like to inform you regarding student attendance. Please follow up with school administration.",
      );
    } else if (templateKey === "exam") {
      setModalNotificationTitle(
        lang === "ar"
          ? "إعلان جدول الاختبارات النهائية"
          : "Final Exam Schedule Announcement",
      );
      setModalNotificationContent(
        lang === "ar"
          ? "تم اعتماد ونشر جدول الاختبارات التقييمية. نرجو الحرص على مراجعة المقررات والالتزام بالحضور في المواعيد المحددة."
          : "The evaluation exam schedule has been published. Please ensure thorough revision and timely attendance.",
      );
    } else if (templateKey === "parents_meeting") {
      setModalNotificationTitle(
        lang === "ar"
          ? "دعوة لاجتماع أولياء الأمور الدوري"
          : "Parents-Teachers Meeting Invitation",
      );
      setModalNotificationContent(
        lang === "ar"
          ? "يسر إدارة المدرسة دعوتكم لحضور الاجتماع الدوري لمناقشة المستوى الأكاديمي والتربوي لأبنائنا الطلاب يوم الخميس القادم."
          : "You are cordially invited to attend the periodic parents meeting next Thursday.",
      );
    } else if (templateKey === "general_announcement") {
      setModalNotificationTitle(
        lang === "ar"
          ? "تعميم إداري هام للجميع"
          : "Important School Announcement",
      );
      setModalNotificationContent(
        lang === "ar"
          ? "تود إدارة رياض ومدارس أنوار العلى الدولية تذكير جميع الطلاب وأولياء الأمور بالتعليمات والأنشطة القادمة."
          : "Anwar Al-Ola Int. Model Schools would like to remind all students & parents of upcoming activities.",
      );
    }
  };
  ...
  const onSendNotificationSubmit = (e) => {
    ...
    if (modalNotificationType === "student") {
      ...
      const classStudents = students.filter(
        (s) => s.grade === modalNotificationGrade,
      );
  ...
  const getNotificationSender = useCallback(
    (notif) => {
  ...
  const formatTime12hMakkah = useCallback(
    (dateStr) => {
      if (!dateStr) return "";
      try {
  ...
  const resolveGradeName = useCallback(
    (notif) => {
      ...
      if (notif.studentId) {
        const targetStudent = students.find(
          (s) => s.id === Number(notif.studentId),
        );
        if (targetStudent) {
          const studentGrade =
            targetStudent.grade ||
            targetStudent.class_name ||
            targetStudent.grade_name;
          if (
            studentGrade &&
            studentGrade !== "null" &&
            studentGrade !== "NULL" &&
      ...
      const classMatch = textToSearch.match(
        /(?:للفصل|فصل|الصف)\s+([^\s:,.-]+(?:\s*[-–]\s*[^\s:,.-]+)?)/,
      );
      if (
        classMatch &&
        classMatch[1] &&
        !classMatch[1].includes("العام") &&
        !classMatch[1].includes("ابنكم")
      ) {
  ...
  const statsAllUsers = useMemo(() => {
    return notifications.filter(
      (n) => n.type === "general" || n.type === "all_users",
    ).length;
  }, [notifications]);

  const statsParents = useMemo(() => {
    return notifications.filter(
      (n) => n.type === "parents" || n.type === "broadcast_parents",
    ).length;
  }, [notifications]);

  const statsTeachers = useMemo(() => {
    return notifications.filter(
      (n) => n.type === "teachers" || n.type === "broadcast_teachers",
    ).length;
  }, [notifications]);
  ...
  const getNotificationRecipient = useCallback(
    (notif) => {
      ...
      if (type === "general" || type === "all_users") {
        return lang === "ar"
          ? "جميع المستخدمين (معلمين وأولياء أمور)"
      ...
      if (type === "student" || type === "private") {
        const targetStudent = students.find(
          (s) => String(s.id) === String(notif.studentId),
        );
        const sName =
          notif.studentName ||
          (targetStudent
            ? targetStudent.name ||
              targetStudent.name_ar ||
              targetStudent.name_en
            : null);
        if (sName)
          return lang === "ar" ? `الطالب: ${sName}` : `Student: ${sName}`;
        return notif.studentId
          ? lang === "ar"
            ? `الطالب (رقم #${notif.studentId})`
      ...
      if (type === "teacher") {
        const targetTeacher = teachers.find(
          (t) => String(t.id) === String(notif.teacherId),
        );
        const tName =
          notif.teacherName ||
          (targetTeacher
            ? targetTeacher.name ||
              targetTeacher.name_ar ||
              targetTeacher.name_en
            : null);
        if (tName)
          return lang === "ar" ? `المعلم: ${tName}` : `Teacher: ${tName}`;
        return notif.teacherId
          ? lang === "ar"
            ? `المعلم (رقم #${notif.teacherId})`
            : `Teacher (#${notif.teacherId})`
          : lang === "ar"
            ? "معلم مخصص"
            : "Teacher";
      }
  ...
  const filteredNotifications = useMemo(() => {
    return notifications.filter((notif) => {
      if (
        filterDate &&
        notif.date &&
        !notif.date.substring(0, 10).startsWith(filterDate)
      )
        return false;
      if (activeFilter === "all_users")
        return notif.type === "general" || notif.type === "all_users";
      if (activeFilter === "parents")
        return (
          notif.type === "parents" ||
          notif.type === "broadcast_parents" ||
          notif.type === "general" ||
  ...
  const getCategoryDetails = (
    ...
    if (
      ...
      return {
        label:
          lang === "ar" ? "تعميم عام لأولياء الأمور" : "All Parents Broadcast",
      ...
      const name =
        lang === "ar"
          ? studentName || "طالب مخصص"
          : studentNameEn || studentName || "Private Student";
      const studentLabel =
        cleanGrade !== "الصف الدراسي" && cleanGrade !== "Class"
          ? lang === "ar"
            ? `طالب (${cleanGrade}): ${name}`
            : `Student (${cleanGrade}): ${name}`
          : lang === "ar"
            ? `طالب: ${name}`
            : `Student: ${name}`;
      return {
      ...
      return {
        label:
          lang === "ar" ? "تعميم لجميع المعلمين" : "All Teachers Broadcast",
      ...
      const name =
        lang === "ar"
          ? teacherName || "معلم مخصص"
          : teacherNameEn || teacherName || "Teacher";
      return {
    ...
    // Default fallback
    const isHomework =
      (notif.title && notif.title.includes("واجب")) ||
      (notif.content && notif.content.includes("واجب"));
    ...
    ? !qStudent
      ...
      : students.filter((s) => {
          ...
          const nameMatch = Boolean(
            s.name?.toString().toLowerCase().includes(qStudent) ||
            s.nameEn?.toString().toLowerCase().includes(qStudent),
          );
          ...
          const studentNum =
            ...
            s.national_id ||
            s.code;
          const studentNumMatch = Boolean(
            studentNum?.toString().toLowerCase().includes(qStudent),
          );
  ...
  const filteredTeachersList = Array.isArray(teachers)
    ? !qTeacher
      ...
      : teachers.filter((teach) => {
          ...
          const nameMatch = Boolean(
            teach.name?.toString().toLowerCase().includes(qTeacher) ||
            teach.nameEn?.toString().toLowerCase().includes(qTeacher),
          );
          ...
          const jobIdMatch = Boolean(
            jobId?.toString().toLowerCase().includes(qTeacher),
          );
    ...
    <div className="notif-command-center">
      ...
      <div className="notif-stats-grid">
        ...
        <div className="notif-stat-card">
          ...
          <div
            className="notif-stat-icon-wrapper"
            style={{
              background: "linear-gradient(135deg, #1e508e 0%, #103058 100%)",
            }}
          >
            <Bell size={22} />
          </div>
        ...
        <div className="notif-stat-card">
          <div className="notif-stat-content">
            ...
            <span className="notif-stat-label">
              {lang === "ar" ? "تعاميم أولياء الأمور" : "Parents Broadcasts"}
            </span>
          </div>
          <div
            className="notif-stat-icon-wrapper"
        ...
        <div className="notif-stat-card">
          <div className="notif-stat-content">
            ...
            <span className="notif-stat-label">
              {lang === "ar" ? "تعاميم المعلمين" : "Teachers Broadcasts"}
            </span>
          </div>
          <div
            className="notif-stat-icon-wrapper"
        ...
        <div className="notif-toolbar-top-row">
          ...
          <div className="notif-search-box">
            ...
            <input
              ...
              placeholder={
                lang === "ar"
                  ? "البحث في سجل الإشعارات المرسلة..."
          ...
          >
            ...
            {canAction("communications", "create") && (
              ...
              >
                ...
                <span>
                  {lang === "ar" ? "إنشاء إشعار فوري" : "Compose Alert"}
                </span>
              </button>
            )}
          </div>
      ...
      >
        ...
        >
          ...
          <span>
            {lang === "ar" ? "سجل الإرسال التاريخي" : "Historical Dispatch Log"}
          </span>
        </h4>
        <span
          style={{
      ...
      {loading ? (
        ...
        >
          ...
          >
            {search
              ? lang === "ar"
                ? "لا توجد نتائج تطابق كلمة البحث"
                : "No notifications match search"
              : lang === "ar"
                ? "لا توجد إشعارات مسجلة في هذا التبويب"
          ...
          {canAction("communications", "create") && (
            ...
            >
              <Plus size={16} />
              <span>
                {lang === "ar" ? "إنشاء إشعار جديد الآن" : "Compose Alert Now"}
              </span>
            </button>
          )}
        </div>
          ...
          >
            ...
            >
              ...
              >
                ...
                <span>
                  {lang === "ar"
                    ? "إرسال ونشر إشعار فوري جديد"
                    : "Compose Instant Announcement"}
                </span>
              </h3>
              <button
            ...
            <form onSubmit={onSendNotificationSubmit}>
              ...
              >
                ...
                >
                  ...
                  <div className="preset-templates-container">
                    <button
                      ...
                      onClick={() =>
                        applyPresetTemplate("general_announcement")
                      }
                    >
                      <FileText
                        size={13}
                      ...
                      />
                      <span>
                        {lang === "ar" ? "تعميم إداري" : "General Notice"}
                      </span>
                    </button>
                    <button
                      type="button"
                    ...
                    <button
                      type="button"
                      className="preset-template-chip"
                      onClick={() => applyPresetTemplate("parents_meeting")}
                    >
                    ...
                    <button
                      type="button"
                      className="preset-template-chip"
                      onClick={() => applyPresetTemplate("absence")}
                    >
                      <AlertCircle size={13} style={{ color: "#d97706" }} />
                      <span>
                        {lang === "ar" ? "تنبيه مواظبة" : "Attendance Alert"}
                      </span>
                    </button>
                  </div>
                </div>
                  ...
                  >
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: "12px",
                        border:
                          modalNotificationType === "all_users"
                            ? "2px solid var(--color-primary-ui)"
                    ...
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: "12px",
                        border:
                          modalNotificationType === "teachers"
                            ? "2px solid var(--color-primary-ui)"
                            : "1px solid var(--color-border)",
                        background:
                          modalNotificationType === "teachers"
                ...
                {modalNotificationType === "student" && (
                  ...
                  >
                    ...
                    <input
                      ...
                      placeholder={
                        lang === "ar"
                          ? "ابحث باسم الطالب أو الرقم الأكاديمي..."
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setModalNotificationStudentId(e.target.value)
                      }
                      className="text-field"
                      style={{
                        minHeight: "45px",
                ...
                {modalNotificationType === "class" && (
                  ...
                  >
                    ...
                    <select
                      value={selectedGrade}
                      onChange={(e) =>
                        setModalNotificationGrade(e.target.value)
                      }
                      className="text-field"
                      style={{
                        minHeight: "45px",
                ...
                {modalNotificationType === "teacher" && (
                  ...
                  >
                    <label style={{ fontSize: "11.5px", fontWeight: "700" }}>
                      🔍 {lang === "ar" ? "إختيار المعلم" : "Select Teacher"}
                    </label>
                    <input
                      type="text"
                      placeholder={
                        lang === "ar"
                          ? "ابحث باسم المعلم أو الرقم الوظيفي..."
                          : "Search by teacher name or Job ID..."
                      }
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setModalNotificationTeacherId(e.target.value)
                      }
                      className="text-field"
                      style={{
                        minHeight: "45px",
                ...
                >
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setModalNotificationContent(e.target.value)
                    }
                    placeholder={
                      lang === "ar"
                        ? "محتوى وتفاصيل البلاغ..."
              ...
              >
                ...
                >
                  <Send size={14} />
                  <span>
                    {lang === "ar" ? "إرسال ونشر الآن" : "Broadcast Now"}
                  </span>
                </button>
              </footer>
            </form>
  ...
  function renderNotificationCard(notif) {
    const student =
      notif.type === "student"
        ? students.find((s) => s.id === Number(notif.studentId))
        : null;
    const resolvedStudentName = student ? student.name : notif.studentName;
    const resolvedStudentNameEn = student
      ? student.nameEn
      : notif.studentNameEn;
    ...
    return (
      ...
      >
        <div className="notif-card-header">
            ...
            <div>
              ...
              <div className="notif-card-meta">
                ...
                >
                  <Clock
                    size={13}
                    style={{ color: "var(--color-primary-ui)", opacity: 0.8 }}
                  />
                  <span>
          ...
          {/* Actions Group */}
          <div
            className="notif-action-btn-group no-print"
          ...
          >
            ...
            <button
              ...
              title={
                lang === "ar" ? "نسخ نص الإشعار" : "Copy notification text"
              }
            >
              {copiedId === notif.id ? (
        ...
        <div className="notif-card-footer">
          ...
          >
            <span>
              ✍️ {lang === "ar" ? "المرسل: " : "Sender: "}
              <strong>{getNotificationSender(notif).name}</strong>
            </span>
            <span>
              🎯 {lang === "ar" ? "الموجه إليه: " : "Recipient: "}
              <strong style={{ color: "var(--color-primary-ui)" }}>
                {getNotificationRecipient(notif)}
              </strong>
            </span>
          </div>
          ...
          >
            ...
            <span>
              {lang === "ar"
                ? "تم النشر كإشعار فوري وتنبيه SMS"
                : "Sent via Push & SMS"}
            </span>
          </div>
        </div>
